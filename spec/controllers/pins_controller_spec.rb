...
describe PinsController do
	...
	describe "GET api/v1/pin/:id" do
		let(:json){JSON.parse(response.body)}

		before do
			user = User.create(username: "Bob123")
			pin = Pin.create(description: "I am a description", item_name: "Pin1", user_id: user.id, buy_sell: true)
	...
	describe "PATCH api/v1/pin/:id" do
		let(:json){JSON.parse(response.body)}

		before do
			user = User.create(username: "Bob123")
			pin = Pin.create(description: "I am a description", item_name: "Pin1", user_id: user.id, buy_sell: true)
			patch :update, id: 1, pin: {description: "New description", item_name: "Pin1", user_id: user.id, buy_sell: true}, format: :json
		end
